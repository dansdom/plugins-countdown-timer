...
(function ($) {
	...
	$.Countdown.settings = {
		...
		finishedFunction : false,	// the user can define a function instead of using a finished method
		digitHeight : 55,
		digitWidth : 55,
	...
	$.Countdown.prototype = {
					...
					{					
						...
						if (counter.minTens == 0)
						{
							...
							if (counter.hourOnes == 0)
							{
										...
										if (counter.dayTens == 0)
										{
											...
											if (counter.monthOnes == 0)
											{
												...
												if (counter.monthTens == 0)
												{
													...
													{
														counter.yearOnes = 10;
														if (this.opts.showYear) { this.el.years.find(ones).css("top", (counter.yearOnes * digit), 500); }													
														...
														{
															counter.yearTens--;
															if (this.opts.showYear) { this.step(this.el.years.find(tens), (counter.yearTens * digit), 500); }
														}
													}
													
													if (this.opts.showMonth) { this.el.months.find(tens).css("top", (counter.monthTens * digit) + "px"); }
													counter.yearOnes--;
													if (this.opts.showYear) { this.step(this.el.years.find(ones), (counter.yearOnes * digit), 500); }
												...
												counter.monthTens--;
												if (this.opts.showMonth) { this.step(this.el.months.find(tens), (counter.monthTens * digit), 500); }
								...
								counter.hourTens--;
								if (this.opts.showHour) { this.step(this.el.hours.find(tens), (counter.hourTens * digit), 500); }
							}
							
							counter.hourOnes--;
							if (this.opts.showHour) { this.step(this.el.hours.find(ones), (counter.hourOnes * digit), 500); }
						}
						
						counter.minTens--;
						if (this.opts.showMinute) { this.step(this.el.minutes.find(tens), (counter.minTens * digit), 500); }
					}
					
					counter.minOnes--;
					if (this.opts.showMinute) { this.step(this.el.minutes.find(ones), (counter.minOnes * digit), 500); }
				}
				
				counter.secTens--;
				if (this.opts.showSecond) { this.step(this.el.seconds.find(tens), (counter.secTens * digit), 500); }
			}
		...
		finish : function()	
		{
			if (this.opts.finishedFunction)
			{
				if (typeof this.opts.finishedFunction == "function")
				{
					this.opts.finishedFunction.call();
				}
			}
			else
			{
